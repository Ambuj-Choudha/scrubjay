...
. "$APP/bin/lib.sh"   # the UI marks; function definitions only, and no config is read here — this
                      # script runs while the config is still the pre-rename dotclaude one
...
[ "$APPLY" = 1 ] || sj_info "DRY RUN — showing what would change. Re-run with --apply to do it."

# 1) config dir ------------------------------------------------------------------------
if [ -d "$OLDC" ] && [ ! -d "$NEWC" ]; then
  sj_step "config dir: $OLDC -> $NEWC"
  run "cp -a '$OLDC' '$NEWC'"
elif [ -d "$NEWC" ]; then sj_ok "config dir already at $NEWC"
else sj_warn "no config dir at $OLDC (nothing to migrate?)"; fi
...
if [ "$APPLY" = 1 ] && [ -f "$CFG" ]; then
  ...
  sed -i -E \
    ...
    "$CFG"
  sj_ok "rewrote $CFG (DOTCLAUDE_->SCRUBJAY_, dir tokens; backup kept)"
else
  sj_step "rewrite $CFG: DOTCLAUDE_->SCRUBJAY_ + dir-name tokens"
fi

# 3) local clone dirs + remotes --------------------------------------------------------
if [ -d "$OLDD" ] && [ ! -d "$NEWD" ]; then
  sj_step "clone base: $OLDD -> $NEWD"; run "mv '$OLDD' '$NEWD'"
elif [ -d "$NEWD" ]; then sj_ok "clone base already at $NEWD"; fi
...
for old in "${!RENAME[@]}"; do
  ...
  if [ -d "$NEWD/$old" ] && [ ! -d "$NEWD/$new" ]; then
    sj_step "clone dir: $NEWD/$old -> $NEWD/$new"; run "mv '$NEWD/$old' '$NEWD/$new'"
  fi
  ...
  if [ -d "$d/.git" ]; then
    ...
    case "$url" in
      *"/$old.git"|*"/$old")
        ...
        sj_step "remote in $d: $url -> $newurl"; run "git -C '$d' remote set-url origin '$newurl'";;
    esac
  fi
done
...
if [ -n "$OLDSTORE" ] && [ -d "$OLDSTORE" ] && [[ "$OLDSTORE" == *dotclaude-storage* ]]; then
  ...
  if [ -e "$NEWSTORE" ]; then sj_ok "storage already at $NEWSTORE"
  elif [ -w "$parent" ]; then
    sj_step "NAS storage: $OLDSTORE -> $NEWSTORE  (archive moves with the rename; inode preserved)"
    run "mv '$OLDSTORE' '$NEWSTORE'"
  else
    sj_warn "cannot rename $OLDSTORE (no write on $parent) — do it with the account that owns it:"
    sj_warn "    mv '$OLDSTORE' '$NEWSTORE'"
  fi
fi

# 5) drop the stale dcmcp MCP registration --------------------------------------------
if command -v claude >/dev/null 2>&1; then
  sj_step "MCP: remove stale 'dcmcp' (sjmcp re-registers via claude-sync)"
...
if [ "$APPLY" = 1 ] && [ ! -d "$APP" ]; then
  case "$APP" in
    "$OLDD"/*) rest="${APP#"$OLDD"/}"; rest="${rest/#dotclaude/scrubjay}"; cand="$NEWD/$rest"
               if [ -d "$cand" ]; then APP="$cand"; sj_ok "app clone moved by migration → $APP"
               else sj_warn "app clone moved and $cand not found — run bin/claude-sync.sh by hand from the moved clone"; fi ;;
    *) sj_warn "app path $APP vanished — run bin/claude-sync.sh by hand from the moved clone" ;;
  esac
fi

# 6) re-apply config into ~/.claude ---------------------------------------------------
sj_step "claude-sync: relink ~/.claude (now /sj* commands) + register sjmcp"
run "'$APP/bin/claude-sync.sh' >/dev/null 2>&1 || true"

echo
if [ "$APPLY" = 1 ]; then sj_ok "this machine migrated."; else sj_info "dry run complete — re-run with --apply."; fi
cat <<EOF
