...
#   env: CHATS_REPO      clone of scrubjay-chats    (default ~/scrubjay-chats)
#        CHATS_REPO_URL  git remote to clone on first run, e.g.
#                        git@github.com:<your-gh-user>/scrubjay-chats.git  (required if not cloned)
#        NAS_DIR         destination on the NAS, e.g. /mnt/nas1/scrubjay-storage  (required)
set -euo pipefail

CHATS_REPO="${CHATS_REPO:-$HOME/scrubjay-chats}"
NAS_DIR="${NAS_DIR:?set NAS_DIR to the NAS mount, e.g. /mnt/nas1/scrubjay-storage}"

[ -d "$CHATS_REPO/.git" ] || \
  git clone "${CHATS_REPO_URL:?set CHATS_REPO_URL to your scrubjay-chats remote on first run}" "$CHATS_REPO"
