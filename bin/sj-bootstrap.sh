...
gh_login()  { sj_has gh && gh api user --jq .login 2>/dev/null; }
...
resolve_owner() {
  ...
  [ -n "$o" ] || sj_die "can't determine your GitHub account — set SCRUBJAY_OWNER=<your-gh-user>"
  ...
  if [ "$o" = "$UPSTREAM_OWNER" ]; then
    ...
    if [ "$gl" != "$UPSTREAM_OWNER" ]; then
      sj_warn "owner resolved to '$UPSTREAM_OWNER' — that's this project's UPSTREAM account, not yours."
      sj_warn "You don't need to fork scrubjay; you need your own private data repos."
      sj_die  "set SCRUBJAY_OWNER=<your-gh-user> and re-run${gl:+ (gh says you are '$gl')}"
    fi
  fi
  printf '%s' "$o"
}
...
ensure_repo() {  # ensure_repo <owner/name> <description>
  local full="$1" desc="$2"
  if remote_has "$full"; then sj_ok "repo exists: $full"; return 0; fi
  if sj_has gh; then
    sj_info "creating private repo $full"
    if gh repo create "$full" --private --description "$desc" >/dev/null 2>&1; then
      sj_ok "created $full (private)"; return 0
    fi
    sj_warn "gh repo create failed for $full (already exists but unreadable? wrong account?)"; return 1
  fi
  sj_warn "private repo $full does not exist (or your SSH key can't read it), and the 'gh' CLI isn't installed."
...
ensure_clone() {  # ensure_clone <owner/name> <dir> <label>
  ...
  if [ -d "$dir/.git" ]; then sj_ok "$label already cloned ($dir)"; return 0; fi
  sj_info "cloning $label → $dir"
  git clone -q "$(ssh_url "$full")" "$dir" 2>/dev/null || { sj_warn "clone failed: $full"; return 1; }
...
seed_data() {  # seed_data <dir>
  local dir="$1"
  if [ -f "$dir/settings/settings.base.json" ]; then sj_ok "scrubjay-data already seeded"; return 0; fi
  sj_info "seeding scrubjay-data from skeleton/data (settings + hooks, claude-md, host/log dirs)"
  cp -r "$APP/skeleton/data/." "$dir/" || { sj_warn "copy of skeleton/data failed"; return 1; }
  if ( cd "$dir" && git add -A \
         && git commit -q -m "seed scrubjay-data (settings + hooks, claude-md, host/log dirs)" \
         && git push -qu origin HEAD ); then
    sj_ok "seeded + pushed scrubjay-data"
  else
    sj_warn "seeded locally, but the commit/push failed — push $dir by hand"
  fi
}
...
seed_chats() {  # seed_chats <dir>
  local dir="$1"
  git -C "$dir" rev-parse --verify HEAD >/dev/null 2>&1 && { sj_ok "scrubjay-chats already has commits"; return 0; }
  sj_info "seeding scrubjay-chats (README + credential guard; transcripts are NOT ignored)"
  printf '%s\n' '# scrubjay-chats' '' \
  ...
  if ( cd "$dir" && git add -A && git commit -q -m "seed scrubjay-chats" && git push -qu origin HEAD ); then
    sj_ok "seeded + pushed scrubjay-chats"
  else
    sj_warn "seeded locally, but the push failed — push $dir by hand"
  fi
}

# ---- --repo mode: ensure ONE repo, print its URL (used by onboard-memory.sh) -----------
if [ "${1:-}" = "--repo" ]; then
  [ -n "${2:-}" ] || sj_die "usage: sj-bootstrap.sh --repo <name>"
...
echo; sj_info "bootstrapping private repos for '$OWNER'  (backend: $BACKEND, base: $BASE)"
...
echo
if [ "$rc" = 0 ]; then
  sj_ok "private repos ready under '$OWNER'"
  sj_info "cross-machine memory gets its own repo — enabled separately by bin/onboard-memory.sh (/sjmemory)"
else
  sj_warn "bootstrap incomplete — create the repo(s) above, then re-run: bin/sj-bootstrap.sh"
fi
exit "$rc"
