...
if [ "$MODE" = authorize ]; then
  cmd="$(role_command "$ROLE" "$ROOT")" || sj_die "unknown role '$ROLE' (use: relay, memory, mcp)"
  [ -n "$KEYSRC" ] || sj_die "give a public key file, or - to read stdin"
  if [ "$KEYSRC" = - ]; then key="$(cat)"; else
    [ -f "$KEYSRC" ] || sj_die "no such file: $KEYSRC"
    key="$(cat "$KEYSRC")"
  fi
  key="$(printf '%s' "$key" | tr -d '\r' | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')"
  case "$key" in
    ssh-ed25519\ *|ssh-rsa\ *|ecdsa-sha2-*\ *|sk-ssh-*\ *) ;;
    *) sj_die "that does not look like an SSH public key (did you pass a PRIVATE key by mistake?)" ;;
  esac
  case "$key" in *PRIVATE*) sj_die "refusing: that is a PRIVATE key. Send me the .pub." ;; esac
  ...
  if grep -qF "$keybody" "$AK" 2>/dev/null && grep -qF "$cmd" "$AK" 2>/dev/null; then
    sj_ok "already authorized for '$ROLE' — nothing to do"; exit 0
  fi
  ...
  printf '%s %s\n' "$cmd" "$key" >> "$AK" || sj_die "could not write $AK"
  sj_ok "authorized for '$ROLE' in $(sj_pretty_path "$AK")"
  sj_info "that key may now ONLY: $(case "$ROLE" in relay) echo 'append to the archive';; memory) echo 'run git against the memory repo';; mcp) echo 'read the archive';; esac)"
  exit 0
fi

# ── provision ─────────────────────────────────────────────────────────────────────────────────
echo; sj_info "scrubjay receiver setup on '$(sj_host)'  (archive root: $ROOT)"
[ "$MODE" = print ] && sj_info "--print: reporting only, changing nothing"

if [ -d "$ROOT" ]; then sj_ok "archive root exists: $ROOT"
elif [ "$MODE" = print ]; then sj_info "would create: $ROOT"
else
  mkdir -p "$ROOT" && chmod 2775 "$ROOT" 2>/dev/null \
    && sj_ok "created $ROOT (setgid, so pushes inherit the group that reads them)" \
    || sj_die "could not create $ROOT"
fi
[ -w "$ROOT" ] || sj_warn "$ROOT is not writable by you — pushes will fail"

# The memory bare repo is onboard-memory.sh's job; don't duplicate the hook here, just check.
if [ -d "$ROOT/memory.git" ]; then sj_ok "memory repo present: $ROOT/memory.git"
else sj_info "no memory repo yet — run bin/onboard-memory.sh on this box (it owns that step)"; fi

# Tools each role needs, reported per role so a partial setup is legible.
echo; sj_info "what each role needs here:"
if command -v rrsync >/dev/null 2>&1 || [ -x /usr/share/doc/rsync/scripts/rrsync ]; then
  sj_ok "relay:  rrsync present"
else
  sj_warn "relay:  rrsync MISSING — ships with rsync on Debian/Ubuntu (sudo apt install rsync), or copy"
  sj_warn "        its scripts/rrsync into your PATH. Without it the transcript relay cannot land."
fi
command -v git-shell >/dev/null 2>&1 && sj_ok "memory: git-shell present" \
  || sj_warn "memory: git-shell MISSING — it ships with git (sudo apt install git)"
command -v uv >/dev/null 2>&1 && sj_ok "mcp:    uv present" \
  || sj_warn "mcp:    uv MISSING — needed only to serve archive queries (https://astral.sh/uv)"

# ── what is left, and who has to do it ────────────────────────────────────────────────────────
echo; sj_info "Let a machine in — run HERE, once per client key (a machine must not authorize itself):"
echo
...
echo
sj_info "Each client prints its own public key at the end of its onboarding. Copy the .pub here"
sj_info "(it is public — mail it, paste it, whatever), then run the line above."

echo; sj_info "Still yours, because they need root and/or the router:"
...
echo
sj_info "Then, from a client:  bin/sj-doctor.sh    (it will say whether this box is reachable)"
